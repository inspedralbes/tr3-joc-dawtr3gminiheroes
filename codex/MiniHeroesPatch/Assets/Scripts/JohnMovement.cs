...
public class JohnMovement : MonoBehaviour, IDamageable
{
    ...
    public bool IsRemotePlayer => isRemotePlayer;
    ...
    private void Start()
    {
        ...
        if (!isRemotePlayer && !MiniHeroesRuntimeMode.IsTraining && PlayerPrefs.HasKey("session_token"))
        {
    ...
    {
        if (isRemotePlayer || isDead || body == null)
        {
            return;
        }
    ...
    {
        if (isRemotePlayer || isDead || sourceTeam == Team)
        {
            return;
        }
    ...
    public void ConfigureAsRemotePlayer()
    {
        isRemotePlayer = true;
    ...
    public MultiplayerState CaptureMultiplayerState()
    {
    ...
    public void ApplyRemoteState(float x, float y, float vx, float vy, bool facingRight, bool running, bool dead, float interpolation)
    {
        if (!isRemotePlayer)
        {
            return;
    ...
    private bool CheckGrounded()
{
    ...
    private void OnGUI()
    {
        if (isRemotePlayer || MiniHeroesRuntimeMode.IsTraining)
        {
            return;
        }
    ...
    private void DrawMenuStats(Rect rect)
    {
        ...
        GUI.Label(new Rect(rect.x + 20f, lineY + lineHeight * 4f, rect.width - 40f, 30f), "Da̱o: " + attack, statStyle);
    ...
    private void ConfigurePhysicsForRemote()
    {
        if (body != null)
        {
            body.linearVelocity = Vector2.zero;
